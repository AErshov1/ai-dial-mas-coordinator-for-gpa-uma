...
class UMSAgentGateway:
    ...
    async def response(
        ...
        cid = self.__get_ums_conversation_id(request=request)
        if not cid:
            cid = await self.__create_ums_conversation()
            stage.append_content(f"New UMS Agent Conversation: {cid}\n\n")

        choice.set_state({_UMS_CONVERSATION_ID: cid})

        req_prompt = request.messages[-1].content
        if additional_instructions:
            req_prompt += f"\n\nAdditional instructions:\n{additional_instructions}"

        try:
            ums_resp= await self.__call_ums_agent(
                conversation_id=cid,
                user_message=req_prompt,
                stage=stage
            )
            return Message(role=Role.ASSISTANT.value, content=ums_resp)
        except Exception as e:
            print(f"[ERROR] UMS Agent: {e}")
            return Message(role=Role.ASSISTANT.value, content=f"[ERROR] UMS Agent Execution Failed: {e}")
    ...
    def __get_ums_conversation_id(self, request: Request) -> Optional[str]:
        ...
        for msg in request.messages:
            if msg.custom_content and msg.custom_content.state:
                ...
                if isinstance(state, dict) and _UMS_CONVERSATION_ID in state.keys():
          ...
          async for line in response.aiter_lines():
              if line.startswith('data: '):
                  ...
                  try:
                      ...
                      print(f"[ERROR] Failed to decode JSON from UMS Agent response: {data_str}")
                      continue
