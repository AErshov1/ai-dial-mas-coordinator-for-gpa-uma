...
class MASCoordinator:
    ...
    async def handle_request(self, choice: Choice, request: Request) -> Message:
        ...
        dial = AsyncDial(base_url=self.endpoint, api_key=request.api_key, api_version='2025-01-01-preview')
        stage = StageProcessor().open_stage(choice=choice, name="coordination_request")
        req = await self.__prepare_coordination_request(client=dial, request=request)
    ...
    async def __prepare_coordination_request(self, client: AsyncDial, request: Request) -> CoordinationRequest:
        ...
        """Send request to LLM to prepare coordination request based on user request and return it as CoordinationRequest model."""
    ...
    def __prepare_messages(self, request: Request, system_prompt: str) -> list[dict[str, Any]]:
        ...
        messages: list[dict[str, Any]] = [{ "role": Role.SYSTEM.value, "content": system_prompt }]
        for msg in request.messages:
            if msg.role == Role.USER.value and msg.custom_content is not None:
                messages.append({ "role": Role.USER.value, "content": msg.content })
            else:
    ...
    async def __handle_coordination_request(
        ...
        match coordination_request.agent_name:
            case AgentName.GPA:
                agent_gateway = GPAGateway(endpoint=self.endpoint)
            case AgentName.UMS:
                agent_gateway = UMSAgentGateway(ums_agent_endpoint=self.ums_agent_endpoint)
            case _:
                raise ValueError(f"Unsupported agent name: {coordination_request.agent_name}")

        resp = await agent_gateway.response(
                        request=request,
                        choice=choice,
                        stage=stage,
        ...
        print(f"=> Response from ({coordination_request.agent_name}) agent: ", resp.content)
        return resp
    ...
    async def __final_response(
        ...
        argumented_content = f"## CONTEXT:\n{agent_message.content}\n\n## USER REQUEST:\n{request.messages[-1].content}"
        messages[-1]["content"] = argumented_content
        ...
        async for chunk in chunks:
          if chunk.choices and chunk.choices[0].delta:
            ...
            if chunk_content := delta.content:
